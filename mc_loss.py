...
class MCLoss(nn.Module):
    ...
    def forward(self, feat, targets):
        ...
        # print('mask',mask.shape,'feat',feat.shape)
...
if __name__ == '__main__':
    ...
    mcloss = MCLoss(num_classes=10,cgroups=[2,8],cnums=[128,32])
    targets = torch.from_numpy(np.arange(10)).long()
    feat = torch.randn((10, 512, 7, 7))
    loss = mcloss(feat, targets)
    print(loss)