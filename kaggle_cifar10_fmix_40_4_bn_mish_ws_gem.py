...
dir_path = '/kaggle/working/cifar10_fmix_40_4_bn_mish_ws_gem'
save_path = dir_path+'/cifar10_fmix_40_4_bn_mish_ws_gem.pkl'
best_path = dir_path+'/best_'+'cifar10_fmix_40_4_bn_mish_ws_gem.pkl'
data_path = '/kaggle/input/cifar10-python'
ema_path = dir_path+'/ema_'+'cifar10_fmix_40_4_bn_mish_ws_gem.pkl'
log_txt = dir_path+'/cifar10_fmix_40_4_bn_mish_ws_gem.txt'
import torch
import torch.nn.functional as F
...
class TPJ(nn.Module):
    def __init__(self, depth, num_classes, widen_factor=1, dropRate=0.2,use_ws=False):
        ...
        self.bn3 = nn.BatchNorm2d(nChannels[3])
...
def swa_train(use_cuda, trainloader, testloader, learning_rate_init):
    ...
    if os.path.exists(best_path):
        checkpoint = torch.load(best_path,map_location='cuda' if use_cuda else 'cpu')
    ...
    if os.path.exists(ema_path):
        checkpoint = torch.load(ema_path,map_location='cuda' if use_cuda else 'cpu')
        start_epoch = checkpoint['epoch']
        # best_prec1 = checkpoint['best_prec1']
        model.load_state_dict(checkpoint['state_dict'],strict=False)
        print('ema loaded')
    ...
    if os.path.exists(save_path):
        checkpoint = torch.load(save_path,map_location='cuda' if use_cuda else 'cpu')
        start_epoch = checkpoint['epoch']
        # best_prec1 = checkpoint['best_prec1']
        model.load_state_dict(checkpoint['state_dict'],strict=False)
    ...
    warm_up_with_cosine_lr = lambda epoch: (epoch + 1) / warm_up_epochs if epoch < warm_up_epochs \
        else max(1e-6,0.5 * (math.cos((epoch - warm_up_epochs) / (epoch_nb- warm_up_epochs) * math.pi) + 1))

    lr_scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=warm_up_with_cosine_lr)
    #
    for epoch in range(start_epoch, epoch_nb):
        model.train()
        correctAll = 0
        totalAll = 0
        for batch, (input, target) in enumerate(trainloader):
            ...
            r = np.random.rand(1)
            if r > 0.2 and r < 1.0 and epoch>100: #cutmix
                ...
                loss = criterion(output, target_a) * lam + criterion(output, target_b) * (1. - lam)#+ \
                       # 1.5 *ce_criterion(output, target_a) * lam + 1.5*ce_criterion(output, target_b) * (1. - lam)
            else:
                output = model(input)
                loss = criterion(output, target)#+1.5*ce_criterion(output, target)
...
#             if batch % 5 == 0:
        ...
        print('TRAIN:    batch=%04d' % batch, optimizer.state_dict()['param_groups'][0]['lr'], '  accuracy=%.2f' % (correctAll / totalAll * 100.0),
              '  epoch=%04d' % epoch)
    ...
    with torch.no_grad():
        for batch, (inputs, targets) in enumerate(testloader):
            ...
            for i in range(targets.shape[0]//100):
                ...
                correctAll += correct
                totalAll += total
                print('using:    batch=%04d' % batch,'lr = %.6f','  accuracy=%.2f' % (correct / total * 100.0), end='\r')
        print('USING:    batch=%04d' % batch, '  accuracy=%.2f' % (correctAll / totalAll * 100.0))
    f = open(log_txt, 'a+')
    line = str(batch) + "  " + str((correctAll / totalAll * 100.0)) + '\n'
    f.write(line)
...
trainloader = data.DataLoader(
    datasets.CIFAR10(root=data_path, train=True, download=True, transform=transform_train), pin_memory=True,
    batch_size=128, shuffle=True, num_workers=4)
testloader = data.DataLoader(
    datasets.CIFAR10(root=data_path, train=False, download=True, transform=transform_test), pin_memory=True,
